...
BACKUP_FIELDS = (
    "source_url", "title", "subtitle", "image_url", "video_url", "material_url",
    "year", "min_players", "max_players", "best_players", "recommended_players",
    "min_time", "max_time", "difficulty", "rating", "age", "category", "location",
    "game_type", "parent_title", "parent_game_id", "description", "created_at",
)


def register_backup_routes(app_module):
    app = app_module.app
    ...
    @app.route("/admin/backup/download", endpoint="admin_backup_download")
    ...
    def admin_backup_download():
        ...
        payload = {
            ...
            "games": [backup_game(game) for game in games],
        }
    ...
    @app.route("/admin/backup/restore", methods=["POST"], endpoint="admin_backup_restore")
    @app_module.admin_required
    def admin_backup_restore():
            ...
            for item, game in restored_items:
                ...
                if parent_title and app_module.normalize_game_type(game.game_type) in app_module.EXPANSION_TYPES:
                    parent = db.scalar(
                        ...
                        .where(app_module.Game.game_type == "base", func.lower(app_module.Game.title) == parent_title.lower())
        ...
        flash(f"백업 복원 완료: 새로 추가 {added}개 · 백업값으로 갱신 {updated}개 · 건너뜀 {skipped}개. 현재 DB에만 있던 게임은 삭제하지 않았습니다.", "success")
        return redirect(url_for("admin"))
